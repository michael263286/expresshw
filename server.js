...
app.get("/api/v1/episodes/:episodeID", (req, res) => {
  ...
  if (episode !== null) {
    ...
    res
      .status(404)
      .json({ error: "could not find episode with id: " + episodeID });
  }
});
...
app.get("/name", (req, res) => {
  ...
  if (search) {
    const searchedName = data.episodes.filter((episode) => episode.name.toLowerCase() == (search.toLowerCase())
    )
    return res.json(searchedName)
  } else {
...
app.get("/api/v1/characters/:characterID", (req, res) => {
  ...
  if (character !== null) {
    ...
    res
      .status(404)
      .json({ error: "could not find episode with id: " + characterID });
  }
});
